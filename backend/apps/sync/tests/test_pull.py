...
def test_pull_contains_rules_family_profile_and_honors_since(api, care_scenario):
...
def test_pull_rejects_invalid_cursor(api, care_scenario, since):
    api.force_authenticate(care_scenario["patient"].user)
    assert api.get("/api/v1/sync/pull/", {"since": since}).status_code == 400


def test_response_to_locally_generated_reminder_materialises_server_record(api, care_scenario):
    patient = care_scenario["patient"]
    api.force_authenticate(patient.user)
...
def test_offline_accessibility_updates_are_validated_and_ordered(api, care_scenario):
    patient = care_scenario["patient"]
    api.force_authenticate(patient.user)
    now = timezone.now()

    def push(settings, updated):
        return api.post(
            "/api/v1/sync/push/",
            {
                "items": [
...
def test_offline_sos_replays_once_with_client_id(api, care_scenario):
    ...
    item = {
        "outbox_id": str(uuid4()), "model": "sos_event",
        "object_id": event_id, "patient_id": str(patient.id),
