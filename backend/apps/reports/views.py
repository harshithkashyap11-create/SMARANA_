...
class ReportView(APIView):
    ...
    def get(self, request, patient_id):
        ...
        try:
            start = parse_date(request.query_params.get("from", start.isoformat()))
            end = parse_date(request.query_params.get("to", end.isoformat()))
        ...
        pdf = (
            emergency_pdf(patient, request.user)
            if self.emergency
            else report_pdf(patient, request.user, start, end)
        )
