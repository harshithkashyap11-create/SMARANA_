...
def _login(api: APIClient, *, login_id: str, pin: str):
    return api.post(
        LOGIN_URL,
...
@pytest.mark.django_db
def test_only_primary_caregiver_can_reset_patient_pin(api: APIClient, care_scenario) -> None:
    patient = care_scenario["patient"]
    credential = patient.user.patient_credential
...
@pytest.mark.django_db
def test_patient_and_doctor_cannot_reset_patient_pin(api: APIClient, care_scenario) -> None:
