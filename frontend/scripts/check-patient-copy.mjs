...
const offenders = [];
for (const file of await files(root)) { if (!/\.(tsx?|json)$/.test(file) || file.includes(".test.")) continue; const text = (await readFile(file, "utf8")).replace(/^import[\s\S]*?;[ \t]*$/gm, ""); const literals = [...text.matchAll(/["'`]([^\n"'`]*\b(?:sync|failed|error)\b[^\n"'`]*)["'`]/gi)]; if (literals.length) offenders.push(`${file}: ${literals[0][1]}`); }
