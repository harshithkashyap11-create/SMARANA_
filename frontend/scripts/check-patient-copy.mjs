...
const offenders = [];
for (const file of await files(root)) { if (!/\.(tsx?|json)$/.test(file) || file.includes(".test.")) continue; const text = await readFile(file, "utf8"); const literals = [...text.matchAll(/["'`]([^"'`]*(?:sync failed|sync error)[^"'`]*)["'`]/gi)]; if (literals.length) offenders.push(`${file}: ${literals[0][1]}`); }
