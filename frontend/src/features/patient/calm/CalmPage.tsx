...
export function CalmPage() {
  ...
  return (
    <section className="space-y-6 text-center">
      ...
      <div
        aria-hidden="true"
        className="mx-auto my-12 h-40 w-40 rounded-full bg-primary/30"
        style={
          reducedMotion
            ? undefined
            : {
                transform:
                  running && cue !== "out" ? "scale(1.35)" : "scale(1)",
                transition: `transform ${running ? durations[step % 3] : 1000}ms ease-in-out`,
              }
        }
      />
      ...
      {running ? (
        <BigButton onClick={() => setStep(-1)}>{t("calm.stop")}</BigButton>
      ) : (
        <BigButton onClick={() => setStep(0)}>
          {t(complete ? "games.playAgain" : "calm.start")}
        </BigButton>
      )}
    </section>
  );
