...
vi.mock("../../../shared/hooks/useTts", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../../../shared/hooks/useTts")>()),
  speak: spoken,
}));
afterEach(() => {
  vi.useRealTimers();
...
test("speaks ordered cues, respects reduced motion, and stops", async () => {
  ...
  expect(spoken).toHaveBeenLastCalledWith(
    "Breathe in",
    expect.objectContaining({ rate: 0.7 }),
  );
  await act(() => vi.advanceTimersByTime(4000));
  expect(spoken).toHaveBeenLastCalledWith("Rest gently", expect.anything());
  await act(() => vi.advanceTimersByTime(4000));
  expect(spoken).toHaveBeenLastCalledWith("Breathe out", expect.anything());
  expect(
