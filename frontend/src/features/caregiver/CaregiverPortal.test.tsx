...
beforeEach(() => {
  vi.stubGlobal("fetch", vi.fn((input: RequestInfo | URL) => {
    ...
    if (url.endsWith("/patients/")) return json([patient, { ...patient, id: "p2", name: "Mira", is_primary: false }]);
    if (url.includes("adherence")) return json({ days: Array.from({ length: 7 }, (_, index) => ({ date: `2026-09-${8 + index}`, reminders: index === 6 ? [{ id: "r1", title: "Morning tablet", category: "medicine", scheduled_at: "2026-09-14T08:00:00Z", status: "taken", responded_at: "2026-09-14T08:05:00Z" }] : [] })), summary: { taken: 1 } });
    return json([]);
  }));
});
...
test("schedule submits expected payload and hides doctor edit action", async () => {
  ...
  fetchMock.mockImplementation((input, init) => {
    if (init?.method === "POST") {
      ...
      return json({ id: "new", ...parsed, source: "caregiver", set_by: "Priya" }, 201);
    }
    return json([{ id: "d1", title: "Prescription", category: "medicine", time_of_day: "09:00:00", days_of_week: [0], start_date: "2026-09-14", end_date: null, icon: "", note: "", source: "doctor", set_by: "Dr. Deka" }]);
  });
  renderWithProviders(<ScheduleTab patientId="p1" />);
  expect(await screen.findByText("🔒 Set by Dr. Deka")).toBeInTheDocument();
  expect(screen.queryByRole("button", { name: "Edit" })).not.toBeInTheDocument();
  fireEvent.change(screen.getByLabelText("Title"), { target: { value: "Evening tea" } });
  ...
  expect(typeof post?.[1]?.body === "string" ? post[1].body : "").toContain('"title":"Evening tea"');
});
