...
export function OverviewTab({ patientId }: { patientId: string }) {
  ...
  return <form key={recordText(query.data?.updated_at)} className="grid gap-2" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); mutation.mutate({ diagnoses: data.get("diagnoses"), assessment_scores: [], visual_limits: data.get("visual_limits"), motor_limits: data.get("motor_limits"), ideal_session_minutes: Number(data.get("ideal_session_minutes")) || null, max_difficulty_level: Number(data.get("max_difficulty_level")) || null }); }}>
