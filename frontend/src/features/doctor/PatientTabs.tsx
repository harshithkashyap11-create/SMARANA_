...
export function DifficultyTab({ patientId }: { patientId: string }) {
  ...
  return <div className="space-y-4">{query.data.map((game) => <article className="rounded border p-4" key={game.game_key}>
    ...
    <form className="mt-3 flex flex-wrap gap-2" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); mutation.mutate({ gameKey: game.game_key, body: { action: data.get("action"), value: data.get("value") ? Number(data.get("value")) : null, reason: data.get("reason") } }); }}>
      ...
      <button className="rounded bg-primary px-3 text-white">Apply</button>
    </form>
  </article>)}</div>;
}
...
export function RoutineTab({ patientId }: { patientId: string }) {
  ...
  return <div className="space-y-4"><h3 className="font-bold">Medications</h3><form className="grid gap-2" onSubmit={submit}>
    <input name="name" placeholder="Medication" required /><input name="dose" placeholder="Dose" required />
    <input name="times" placeholder="08:00, 20:00" required /><textarea name="instructions" placeholder="Instructions" />
    <button className="rounded bg-primary p-2 text-white">Add medication</button>
    ...
    <form className="grid gap-2" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); assign.mutate({ game: data.get("game"), start_level: Number(data.get("start_level")), target_minutes: Number(data.get("target_minutes")), times_per_week: Number(data.get("times_per_week")), time_slot: data.get("time_slot"), review_date: data.get("review_date"), active: true, notes: data.get("notes") }); }}>
      ...
      <input name="review_date" type="date" required aria-label="Review date" /><textarea name="notes" placeholder="Assignment notes" />
      <button className="rounded bg-primary p-2 text-white">Assign exercise</button>
    </form>
...
export function NotesTab({ patientId }: { patientId: string }) {
  ...
  return <div className="space-y-4"><form className="grid gap-2" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); mutation.mutate({ category: data.get("category"), status_summary: data.get("status"), text: data.get("text"), visibility: data.get("visibility") }); }}>
    <select name="category"><option value="general">General</option><option value="cognitive">Cognitive</option><option value="medication">Medication</option></select>
    <input name="status" placeholder="Status summary" /><textarea name="text" placeholder="Doctor note" required />
    <select name="visibility"><option value="doctor_only">Doctor only</option><option value="care_team">Care team</option><option value="patient_visible">Patient visible</option></select>
    <button className="rounded bg-primary p-2 text-white">Save note</button>
...
export function OverviewTab({ patientId }: { patientId: string }) {
  ...
  return <form key={recordText(query.data?.updated_at)} className="grid gap-2" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); mutation.mutate({ allergies: data.get("allergies"), diagnoses: data.get("diagnoses"), assessment_scores: [], visual_limits: data.get("visual_limits"), motor_limits: data.get("motor_limits"), ideal_session_minutes: Number(data.get("ideal_session_minutes")) || null, max_difficulty_level: Number(data.get("max_difficulty_level")) || null }); }}>
    ...
    <input name="visual_limits" defaultValue={recordText(query.data?.visual_limits)} placeholder="Visual limits" /><input name="motor_limits" defaultValue={recordText(query.data?.motor_limits)} placeholder="Motor limits" />
    <input name="ideal_session_minutes" defaultValue={recordInput(query.data?.ideal_session_minutes)} type="number" placeholder="Ideal session minutes" /><input name="max_difficulty_level" defaultValue={recordInput(query.data?.max_difficulty_level)} type="number" placeholder="Maximum difficulty" />
    <button className="rounded bg-primary p-2 text-white">Save baseline</button>
  </form>;
}
