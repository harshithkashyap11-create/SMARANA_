...
export const isLanguageSwitchRequest = (utterance: string): boolean =>
  /(switch|change).*(language|bengali|assamese|english)/u.test(normalize(utterance));
export function route(utterance: string, _language: VoiceLanguage, context: FamilyContext = {}): RoutedIntent | null {
  ...
  const section = text.match(/(?:open|show|go to) (?:my )?(memories|medicines|routine|games|calm time|people|settings)/u);
  if (section?.[1]) return make("open_section", { section: section[1].replace(" ", "-") });
  ...
  const requestedSection = regionalSection.find(([pattern]) => pattern.test(text));
  if (requestedSection && /(?:খোল|খুল|দেখ|যাও|যান)/u.test(text)) return make("open_section", { section: requestedSection[1] });
  ...
  const reminder = text.match(/remind me to (.+?) at ([0-9]{1,2}(?::[0-9]{2})?)/u);
  if (reminder?.[1] && reminder[2]) return make("set_reminder", { title: reminder[1], time: reminder[2] }, true);
  const regionalReminder = text.match(/(?:মনত পেল|মনে কর).+? (.+?) (?:at|বজাত|টায়) ([0-9]{1,2}(?::[0-9]{2})?)/u);
  if (regionalReminder?.[1] && regionalReminder[2]) return make("set_reminder", { title: regionalReminder[1], time: regionalReminder[2] }, true);
  if (isLanguageSwitchRequest(text)) return context.languageLocked ? null : make("switch_language");
  const call = text.match(/(?:call|phone|ফোন কৰক|ফোন কর|কল কৰক|কল কর) (?:my )?(.+)/u); if (call?.[1]) { const wanted = call[1]; const person = context.familyMembers?.find((p) => [p.name, p.relationship ?? ""].some((x) => distance(normalize(x), wanted) / Math.max(x.length, wanted.length) <= 0.3)); if (person) return make("call_person", { name: person.name }, true); }
  const game = text.match(/(?:play|start|খেল|খেলা) (?:a |the )?(.*?)(?: game)?$/u); if (game) return make("start_game", game[1] ? { game: game[1] } : {});
  return null;
}
