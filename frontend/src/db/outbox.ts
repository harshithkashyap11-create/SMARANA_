...
export function createOutboxEntry(
  ...
  const deviceUpdatedAt =
    typeof candidate === "string" ? candidate : now.toISOString();
  return {
    ...
    patientId,
    payload,
    idempotencyKey: makeIdempotencyKey(model, objectId, deviceUpdatedAt),
...
export async function markRejected(
  ...
  if (!item) return;
  await db.transaction("rw", db.outbox, db.outboxDead, async () => {
    await db.outboxDead.put({
      ...item,
      code,
      lastError: message,
...
export async function markRetry(
  ...
  const delay = delays[Math.min(attempts - 1, delays.length - 1)] ?? 21_600_000;
...
export const isFakeOffline = (): boolean =>
  import.meta.env.VITE_FAKE_OFFLINE === "1";
